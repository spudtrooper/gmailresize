...
function normalizeRules(rules) {
  return [...rules]
    ...
    .filter((rule) => Number.isFinite(rule.minWidth) && Number.isFinite(rule.maxWidth) && VALID_PAGE_SIZES.has(rule.pageSize))
    .sort((a, b) => a.minWidth - b.minWidth);
}

function pickPageSize(width, rules) {
  const match = rules.find((rule) => width >= rule.minWidth && width <= rule.maxWidth);
...
async function handleWidthReport(message) {
  ...
  const [tab] = await chrome.tabs.query({ active: true, lastFocusedWindow: true, url: 'https://mail.google.com/*' });
  ...
  try {
    ...
    if (settingsTab?.id) {
      try { await chrome.tabs.remove(settingsTab.id); } catch (_) {}
    }
  }
}
...
function automationScript(targetPageSize, localeMode) {
  ...
  function clickIfFound(predicate) {
    const el = Array.from(document.querySelectorAll('button, div[role="button"], span, a')).find(predicate);
    if (el) {
      el.click();
      return true;
    }
  ...
  async function setPageSize() {
    ...
    let targetSelect = selects.find((select) => {
      const nearby = [select, select.parentElement, select.closest('tr'), select.closest('div')]
        .filter(Boolean)
        .map(getText)
        .join(' ');
    ...
    if (!targetSelect) {
      ...
      const row = rows.find((node) => /maximum page size/i.test(getText(node)) && node.querySelector('select'));
      targetSelect = row?.querySelector('select') || null;
    }

    if (!targetSelect) {
      return { ok: false, error: 'Could not find the Maximum page size dropdown. Gmail UI may have changed.' };
    }

    const desired = String(targetPageSize);
    const option = Array.from(targetSelect.options).find((opt) => opt.value === desired || getText(opt).includes(desired));
    if (!option) {
      return { ok: false, error: `Could not find page size option ${targetPageSize}.` };
    }
  ...
  return (async () => {
    if (localeMode !== 'english') {
      return { ok: false, error: 'This version only supports English Gmail settings labels.' };
    }
