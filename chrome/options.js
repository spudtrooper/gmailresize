...
async function load() {
  ...
  document.getElementById('automationDelayMs').value = current.automationDelayMs;
  rulesBody.innerHTML = '';
  (current.rules || DEFAULTS.rules).forEach((rule) => rulesBody.appendChild(createRuleRow(rule)));
}
...
function validate(rules) {
  ...
  for (const rule of rules) {
    if (!Number.isFinite(rule.minWidth) || !Number.isFinite(rule.maxWidth) || rule.minWidth > rule.maxWidth) {
...
async function save() {
  ...
  await chrome.storage.sync.set({
    ...
    automationDelayMs: Number(document.getElementById('automationDelayMs').value) || DEFAULTS.automationDelayMs,
    rules,
    localeMode: 'english'
  });
...
document.getElementById('addRule').addEventListener('click', () => {
  rulesBody.appendChild(createRuleRow({ minWidth: 0, maxWidth: 0, pageSize: 25 }));
});
